                        ...
                        Section -Style Heading3 'Security Settings' {
                            ...
                            Section -Style Heading4 'User Details' {
                                ...
                                if ($InfoLevel.Cluster -in (1, 2)) {
                                    ...
                                    @{ Name = 'Permissions'; Expression = { Get-RubrikUserRole -id $_.id | Select-Object @{Name = 'Perms'; Expression = { "ReadOnlyAdmin = $($_.readOnlyAdmin)`nAdmin = $($_.admin)`nOrgAdmin = $($_.orgAdmin)`nManagedVolumeAdmin = $($_.managedVolumeAdmin)`nOrganization=$($_.organization)`nManagedVolumeUser = $($_.managedVolumeUser)`nendUser = $($_.endUser)" } } } } | Sort-Object -Property Username
                                    $UserDetails = @()
